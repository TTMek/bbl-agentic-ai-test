...
This module implements the RAG retrieval layer: it loads the knowledge base,
splits it into paragraph-level chunks, and ranks those chunks against a query
using Term Frequency overlap scoring.
"""
...
def _tokenize(text: str, unique: bool = False) -> list[str] | set[str]:
    """Lowercase, strip punctuation, and drop stop words."""
    words = re.findall(r"[a-z0-9]+", text.lower())
    filtered = [word for word in words if word not in STOP_WORDS and len(word) > 2]
    return set(filtered) if unique else filtered
...
def search_knowledge_base(query: str, top_k: int = 3) -> str:
    query_tokens = _tokenize(query, unique=True)
    if not query_tokens:
        return "No searchable terms found in the query."

    scored: list[tuple[float, str]] = []
    for chunk in load_chunks():
        chunk_tokens_list = _tokenize(chunk, unique=False)
        # Score based on how many times any query token appears in the chunk
        overlap_score = sum(chunk_tokens_list.count(q) for q in query_tokens)
        
        if overlap_score > 0:
            # Normalize by chunk length to prevent bias towards very long paragraphs
            normalized_score = overlap_score / (len(chunk_tokens_list) + 1)
